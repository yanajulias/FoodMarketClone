...
class LoginAccountRemoteUseCaseImpl @Inject constructor(
    private val repository: AuthRepository
) : LoginAccountRemoteUseCase {
    override suspend fun loginAccount(email: String, password: String): Flow<UiResult<User>> =
        flow {
            try {
                emit(UiResult.Loading())
                repository.login(email = email, password = password).collect { result ->
